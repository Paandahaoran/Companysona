...
function draftOption(label, lane, extraBonus = {}) {
  const lanes = {
    doc: option(label, "doc", { data: 2, result: 1 }, { report: 1, risk: 1 }, { bytedance: 2, dji: 1, ...extraBonus }),
    war: option(label, "war", { break: 2, result: 2 }, { packaging: 1, execution: 1 }, { alibaba: 2, pdd: 1, tesla: 1, ...extraBonus }),
    soft: option(label, "soft", { craft: 2, break: -1 }, { emotion: 1, collaboration: 1, packaging: 1 }, { tencent: 2, nio: 1, loreal: 1, popmart: 1, ...extraBonus }),
    process: option(label, "process", { data: 1, break: -2, craft: 1 }, { process: 2, risk: 1 }, { "civil-servant": 2, banker: 1, teacher: 1, ...extraBonus }),
  };
  return lanes[lane];
}
...
const questions = [
  q("汇报", "明天老板汇报，数据能讲但不好看，你会？",
    draftOption("开文档补证据链", "doc"),
    draftOption("包装成阶段性战役", "war"),
    draftOption("先把 PPT 顺到能看", "soft", { tencent: 2 }),
    draftOption("先确认口径能不能说", "process")),
  q("汇报", "老板在会上问：“所以你要什么支持？”",
    draftOption("直接列 blocker 和 owner", "doc", { bytedance: 1, huawei: 1 }),
    draftOption("上升到组织协同问题", "war", { alibaba: 2 }),
    draftOption("先看气氛再递诉求", "soft", { tencent: 2 }),
    draftOption("回去补材料再请示", "process", { "civil-servant": 2 })),
  q("汇报", "项目复盘第一页，你会先写什么？",
    draftOption("假设错在哪，下轮怎么试", "doc", { bytedance: 2 }),
    draftOption("核心抓手和打法沉淀", "war", { alibaba: 2 }),
    draftOption("团队努力和外部变量", "soft", { tencent: 1, nio: 1 }),
    draftOption("基本情况与存在问题", "process", { "civil-servant": 2 })),
  q("汇报", "临时被抓去 10 分钟汇报，你先做什么？",
    draftOption("列结论、证据、风险", "doc", { dji: 2, doctor: 1 }),
    draftOption("起一个能装住全局的词", "war", { alibaba: 2 }),
    draftOption("做 5 页能看的 PPT", "soft", { tencent: 2 }),
    draftOption("问清参会范围和口径", "process", { banker: 1 })),
  q("汇报", "材料里有一页你也觉得虚，但删了成果变少，你会？",
    draftOption("删，别给自己埋雷", "doc", { dji: 2, apple: 1 }),
    draftOption("改成阶段性探索", "war", { alibaba: 2 }),
    draftOption("换个更顺的表达", "soft", { loreal: 2 }),
    draftOption("先给上级看一眼", "process", { "civil-servant": 2 })),
  q("推进", "跨部门同事突然甩锅，说卡点在你这边，你会？",
    draftOption("拉时间线和证据链", "doc", { dji: 2 }),
    draftOption("重新定义协同边界", "war", { alibaba: 1, huawei: 1 }),
    draftOption("私聊缓和一下关系", "soft", { tencent: 2 }),
    draftOption("先同步纪要别扩散", "process", { "civil-servant": 2 })),
  q("推进", "群里讨论半天没人拍板，你会？",
    draftOption("写结论让大家确认", "doc", { bytedance: 2 }),
    draftOption("把问题升维成打法", "war", { alibaba: 2 }),
    draftOption("做一版更好看的方案", "soft", { tencent: 1, apple: 1 }),
    draftOption("提醒先走请示流程", "process", { "civil-servant": 2 })),
  q("推进", "需求突然改口径，但排期不变，你会？",
    draftOption("先把变更点写清楚", "doc", { bytedance: 2 }),
    draftOption("借势重拆一版打法", "war", { alibaba: 2 }),
    draftOption("先安抚，再换个讲法", "soft", { nio: 1, tencent: 1 }),
    draftOption("等正式口径再动作", "process", { "civil-servant": 2 })),
  q("推进", "资源卡在别的部门，你会先怎么动？",
    draftOption("列依赖和截止时间", "doc", { bytedance: 1, huawei: 1 }),
    draftOption("找关键人拉齐盘子", "war", { alibaba: 2 }),
    draftOption("做一版对方好接的材料", "soft", { tencent: 1, loreal: 1 }),
    draftOption("先补请示和流转路径", "process", { "civil-servant": 2 })),
  q("推进", "项目推进太慢，群里开始焦虑，你会？",
    draftOption("把 blocker 全列出来", "doc", { bytedance: 2 }),
    draftOption("重新定一个冲刺节奏", "war", { tesla: 1, pdd: 1 }),
    draftOption("先把信心稳住", "soft", { nio: 2 }),
    draftOption("先发一版会议纪要", "process", { "civil-servant": 2 })),
  q("复盘", "一个方案很有感觉，但数据撑不住，你会？",
    draftOption("先小流量验证", "doc", { bytedance: 2, dji: 1 }),
    draftOption("先讲用户心智", "war", { alibaba: 1, loreal: 1 }),
    draftOption("先把故事讲顺", "soft", { popmart: 2 }),
    draftOption("先别急着表态", "process", { banker: 1 })),
  q("复盘", "会上有人说“这个不够有抓手”，你会？",
    draftOption("问具体指标卡在哪", "doc", { bytedance: 1 }),
    draftOption("立刻拆成三层抓手", "war", { alibaba: 3 }),
    draftOption("换个更有感的表达", "soft", { loreal: 1, popmart: 1 }),
    draftOption("记下来会后再研究", "process", { "civil-servant": 1 })),
  q("复盘", "复盘写到一半，发现锅很难分，你会？",
    draftOption("按事实链拆责任", "doc", { dji: 1, bytedance: 1 }),
    draftOption("讲组织协同没拉齐", "war", { alibaba: 2 }),
    draftOption("把叙事写得体面点", "soft", { tencent: 2 }),
    draftOption("改成“原因较复杂”", "process", { "civil-servant": 2 })),
  q("复盘", "老板说“沉淀一下方法论”，你会？",
    draftOption("先把数据和样本补齐", "doc", { dji: 2 }),
    draftOption("写成一套可复制打法", "war", { alibaba: 2 }),
    draftOption("做成适合传播的模板", "soft", { popmart: 1, loreal: 1 }),
    draftOption("先形成初步材料", "process", { "civil-servant": 2 })),
  q("复盘", "失败项目要对外同步，你会？",
    draftOption("讲清假设被证伪", "doc", { bytedance: 1, dji: 1 }),
    draftOption("转成阶段性探索", "war", { alibaba: 2 }),
    draftOption("先保住团队观感", "soft", { tencent: 1, nio: 1 }),
    draftOption("控制措辞和范围", "process", { "civil-servant": 2, banker: 1 })),
  q("高压", "周日晚收到“明早过一下”，你第一反应是？",
    draftOption("先开文档列风险", "doc", { bytedance: 1 }),
    draftOption("看看能不能变成机会", "war", { alibaba: 1, tesla: 1 }),
    draftOption("做个版本给对方安心", "soft", { tencent: 1 }),
    draftOption("先问这事急到哪级", "process", { banker: 1 })),
  q("高压", "半夜临时高优插进来，你会？",
    draftOption("问清指标和截止时间", "doc", { bytedance: 2 }),
    draftOption("直接拉战役节奏", "war", { alibaba: 1, huawei: 1 }),
    draftOption("先做个能看的版本", "soft", { tencent: 1 }),
    draftOption("先看领导怎么定调", "process", { "civil-servant": 2 })),
  q("高压", "明明不是你的锅，但锅正在滚过来，你会？",
    draftOption("立刻补证据链", "doc", { dji: 2 }),
    draftOption("把边界重新定义", "war", { alibaba: 1, huawei: 1 }),
    draftOption("私聊关键人降温", "soft", { tencent: 2 }),
    draftOption("先留痕别硬刚", "process", { "civil-servant": 2 })),
  q("高压", "领导说“简单弄一下”，你听到的是？",
    draftOption("需求还没定义清楚", "doc", { dji: 1, bytedance: 1 }),
    draftOption("这里有个新抓手", "war", { alibaba: 2 }),
    draftOption("需要先做得像回事", "soft", { tencent: 1, apple: 1 }),
    draftOption("先别理解过度", "process", { "civil-servant": 2 })),
  q("高压", "连续三天都在救火，你会？",
    draftOption("找根因和复发条件", "doc", { dji: 1, doctor: 1 }),
    draftOption("把救火升级成专项", "war", { alibaba: 2 }),
    draftOption("先让大家撑过这波", "soft", { nio: 2 }),
    draftOption("做好记录等复盘", "process", { "civil-servant": 2 })),
  q("高压", "计划排得好好的，领导一句话全改了，你会？",
    draftOption("先重算影响范围", "doc", { bytedance: 1 }),
    draftOption("顺势改成新战役", "war", { alibaba: 2 }),
    draftOption("先稳住大家情绪", "soft", { nio: 2, tencent: 1 }),
    draftOption("立刻更新口径和材料", "process", { "civil-servant": 3 })),
  q("协作", "会前会、会中会、会后会都来了，你会？",
    draftOption("整一份唯一事实源", "doc", { bytedance: 2 }),
    draftOption("把各方诉求装进框架", "war", { alibaba: 2 }),
    draftOption("做一版大家都能接受的表达", "soft", { tencent: 2 }),
    draftOption("分别确认口径别串台", "process", { "civil-servant": 2 })),
  q("协作", "同事说“你这个太细了”，你会？",
    draftOption("细是为了别返工", "doc", { dji: 2 }),
    draftOption("颗粒度决定打法", "war", { alibaba: 2 }),
    draftOption("那我换个好懂的说法", "soft", { teacher: 1, tencent: 1 }),
    draftOption("先按领导口径来", "process", { "civil-servant": 2 })),
  q("协作", "新需求听起来很玄，你会？",
    draftOption("问有没有真实样本", "doc", { dji: 2 }),
    draftOption("先拆用户心智链路", "war", { alibaba: 1, loreal: 1 }),
    draftOption("先画个故事板看看", "soft", { popmart: 2 }),
    draftOption("等需求方补材料", "process", { "civil-servant": 1, banker: 1 })),
  q("协作", "项目快上线了，突然有人提大改，你会？",
    draftOption("看影响范围和证据", "doc", { bytedance: 1, dji: 1 }),
    draftOption("判断能不能变成战役", "war", { alibaba: 2 }),
    draftOption("先做视觉和叙事补救", "soft", { tencent: 1, loreal: 1 }),
    draftOption("没批示先别动主线", "process", { "civil-servant": 2 })),
];
